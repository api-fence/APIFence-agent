...
export class APIFenceAgent {
  private static config: APIFenceConfig;
  private static queue: APIFenceEvent[] = [];
  private static timer: NodeJS.Timeout;

  static init(config: APIFenceConfig) {
    this.config = {
  ...
  static addEvent(event: APIFenceEvent) {
    if (!this.config) {
      return;
    }
  ...
  private static transformEvent(event: APIFenceEvent): any {
